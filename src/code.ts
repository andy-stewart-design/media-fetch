	...
	const imageData: MediaEntry[][] = data.map((set) => {
		if (set.service === 'PEXELS') {
			return set.photos.map((image: PexelsResponse) => {
				...
				let trueWidth = width > height ? 1920 : 1280;
				let trueHeight = height * (trueWidth / width);
				if (trueHeight > 2000) {
					trueWidth = 1080;
					trueHeight = height * (trueWidth / width);
				}
				const orientation = width > height ? 'landscape' : 'portrait';
				const src = image.src.large2x.replace(/w=([^"]*)/, `w=${trueWidth}`).replace(/h=([^"]*)&/, `h=${trueHeight}&`);
				console.log(image.photographer, src);
